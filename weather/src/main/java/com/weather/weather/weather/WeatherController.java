...
public class WeatherController {
    ...
    public String getTemp() {
        ...
        var response1=   webClient.get()
                        .uri("http://api.weatherapi.com/v1/current.json?key="+apiKey+"&q=Milwaukee&aqi=no")
                        .retrieve().bodyToMono(String.class).block();
        var response2=   webClient.get()
                .uri("http://api.weatherapi.com/v1/current.json?key="+apiKey+"&q=Czerwonak&aqi=no")
                .retrieve().bodyToMono(String.class).block();
        var response3=   webClient.get()
                .uri("http://api.weatherapi.com/v1/current.json?key="+apiKey+"&q=Siberia&aqi=no")
                .retrieve().bodyToMono(String.class).block();
